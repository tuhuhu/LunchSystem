...
var ContractSES = (function () {
    return {
        ValidateData: ValidateData,
        OnChangeStartDateContract: OnChangeStartDateContract,
        OnChangeEndDateContract: OnChangeEndDateContract,
        OnChangeTargetYearMonth: OnChangeTargetYearMonth,
        CheckDuplicateTargetYm: CheckDuplicateTargetYm,
        ...
        OnChangeClosingDayOA : OnChangeClosingDayOA,
    ...
    function ValidateData() {
        ...
        // Tax rate
        var TaxRateOp = $('#CONTRACT_OP_TAX_RATE').val();
    ...
    // Validate #CONTRACT_PRJ_PERIOD_START
    function OnChangeStartDateContract() {
    ...
    // Validate #CONTRACT_PRJ_PERIOD_END
    function OnChangeEndDateContract() {
        var endDate = $('#CONTRACT_PRJ_PERIOD_END').val();
        var errInvalid
        ...
        errInvalid = iseiQ.utility.validDate(endDate, Constant.DATE_FORMAT, Constant.WORK_PERIOD_END);
        ...
        if (startDate.length > Constant.MAX_DATE) { // valid string length
            var errInvalid = Constant.WORK_PERIOD_START + Constant.ERR_INCORRECT_DATE;
            iseiQ.utility.showMessageModal(errInvalid, true);
            return;
        }
    ...
    function CheckDuplicateTargetYm(control)
    {
        var targetYM = $(control).val();
        $(control).parents('tbody').find('tr:not(.hide)').find('.target-ym').each(function () {
            if (!$(this).is($(control)))
            {
                var tmpTargetYM = $(this).val();
                if (iseiQ.utility.validDate(tmpTargetYM, Constant.YM_FORMAT, Constant.TARGET_YM) == null
                    && tmpTargetYM == targetYM) {
                    iseiQ.utility.showMessageModal(Constant.ERR_TARGETYM_DUPLICATE, true);
                }
            }
        });
    }
    ...
    function ResetInitValueCopd($parentElement) {
        ...
        $tbCenterLastRow.find('.unit-time-copd').val($('#CONTRACT_OP_UNIT_TIME').val());

        $parentElement.find('.col-right tr:last .del-flg').val(0);
    }

    // Onchange #CONTRACT_OA_CLOSING_DAY
    function OnChangeClosingDayOA(control) {
        var closingDayOA = $(control).val();
        var txtclosingDayOA = '';
        if (closingDayOA == 31)
        {
            txtclosingDayOA = '末';
        }
        else if (closingDayOA != 0)
        {            
            txtclosingDayOA = closingDayOA.toString();
        }
        
        $('.closing-day-op').each(function () {
            $(this).text(txtclosingDayOA);
            $(this).parent().find('.hdd-closing-day-op').val(closingDayOA);
        });
    ...
    function OnChangeCoadPaymentMethodType(control) {
        ...
        if (time_index == TimeIndex.CONTRACT_PERIOD_START) {
            ...
            $parent.find('.unit-time, .lbUnitTime').css('opacity', '1').prop('disabled', false);
        }
        else {
            ...
            $parent.find('.unit-time, .lbUnitTime').css('opacity', '0').prop('disabled', true);
    ...
    //// OnChange #CONTRACT_OA_TAX_RATE, #IN_TAX_1, #IN_TAX_2
    //function OnChangeContractOATaxRate()
    //{
    //    CalculateContAmountFromOA();
    //}

    //// OnChange #CONTRACT_OP_TAX_RATE, #IN_TAX_3, #IN_TAX_4
    //function OnChangeContractOPTaxRate() {
    //    CalculateContAmountFromOP();
    //}
    ...
    function CalcExceedDedutionUnitPrice(refObj, calculation_type, unit_price, base_time_lower, base_time_upper, is_included_tax, tax_rate)
    {
        if (is_included_tax)
        {
            unit_price = unit_price / (1 + tax_rate / 100);
        }
    ...
    //Caculate OP cont amount from OP
    //function CalculateContAmountFromOP() {
    //    var contractoa_unitprice = iseiQ.utility.convertMoneyToInt($('#CONTRACT_OP_UNIT_PRICE').val());
    //    var quantity = $('#CONTRACT_OP_QTY').val();

    //    //if (!iseiQ.utility.validDecimalByType(quantity, Constant.DECIMAL_7_2)) {
    //    //    quantity = 0;
    //    //    $('#CONTRACT_OP_QTY').val(0);
    //    //}

    //    var amount = iseiQ.utility.roundByType($('#RoundingType').val(), contractoa_unitprice * quantity);

    //    var stramount = iseiQ.utility.convertIntToMoney(amount);
    //    $('#CONTRACT_OP_CONT_AMOUNT').val(stramount);
    //}

    //// Caculate OA cont amount from OA
    //function CalculateContAmountFromOA() {
    //    var quantity = $('#CONTRACT_OA_QTY').val();

    //    //if (!iseiQ.utility.validDecimalByType(quantity, Constant.DECIMAL_7_2)) {
    //    //    quantity = 0;
    //    //    $('#CONTRACT_OA_QTY').val(0);
    //    //}

    //    var contractoa_unitprice = iseiQ.utility.convertMoneyToInt($('#CONTRACT_OA_UNIT_PRICE').val());
    //    var amount = iseiQ.utility.roundByType($('#RoundingType').val(),contractoa_unitprice * quantity);

    //    var stramount = iseiQ.utility.convertIntToMoney(amount);
    //    $('#CONTRACT_OA_CONT_AMOUNT').val(stramount);
    //}
}());