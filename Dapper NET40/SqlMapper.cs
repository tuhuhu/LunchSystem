...
this IDbConnection cnn, string sql, object param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
...
)
        {
            ...
            {                
                ...
                string paramValue = strParam.Substring(2, strParam.Length - 4);
                List<string> debitList = paramValue.Split(',').ToList();
                debitList = debitList.Select(x => x.Replace("= ", "= '") + "'").ToList();
                StringBuilder strlog = new StringBuilder();
                if (debitList.Count > 0)
                {
                    strlog.AppendLine("パラメータ {");
                    for (int i = 0; i < debitList.Count; i++)
                    {
                        if (i == 0)
                        {
                            strlog.AppendLine(" SET @" + debitList[i] + " ;");
                        }
                        else
                        {
                            strlog.AppendLine(" SET @" + debitList[i].Substring(1) + " ;");
                        }
                    }
                    strlog.AppendLine(" }");
                }
                parameterInfo = strlog.ToString();
            }
...
this IDbConnection cnn, string sql, object param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        /// <returns></returns>
        public static string GetParameterInfo(object param = null)
        {
